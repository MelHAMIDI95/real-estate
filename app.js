    ...
    if ( siteurl ) {
        ...
        request( siteurl, function ( error, response, html ) {
            if ( !error && response.statusCode == 200 ) {
                ...
                var classeEnergie = $( ' span.value' ).get()[6].children[0].data.replace( /\s/g, '' );
                console.log( classeEnergie );


                //Deuxieme site : meilleursagents
                request( 'https://www.meilleursagents.com/prix-immobilier/' + ville, function ( error, response, html ) {
                    if ( !error && response.statusCode == 200 ) {
                        var $ = cheerio.load( html );
